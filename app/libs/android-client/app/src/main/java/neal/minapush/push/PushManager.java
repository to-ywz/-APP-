...
public class PushManager {
    ...
    public boolean Connect() {
        ...
        if(!NetworkUtil.isNetworkConnect()|| connector==null){
            //NetworkUtil.isNetworkConnect()的返回值为true，存在网络连接
            // connector的返回值不为空连接已建立
            return false;
        }
    ...
    public boolean sendMessage(ClientPushMessage clientPushMessage){
        ...
        WriteFuture writeFuture=session.write(clientPushMessage.UseMsgValue());
        if(writeFuture==null){
            return false;
        }
